...
sub CheckForIndex($$$)
{
    ...
    while(1)
    {
        ...
        my @row_position;
        $position += read $fh, @row_position[0], 4;
        $position += read $fh, @row_position[1], 4;

        $row_position[0] = unpack("i", $row_position[0]);
        $row_position[1] = unpack("i", $row_position[1]);
    
        my $new_val = join "", @row_position;

        print $new_val, "\n";
        print "The numb: ".$row_position[0]." ".$row_position[1]."\n";
        ...
        $$indexCache{$index_file_name}{$unpacked}[0] = $new_val;
...
sub Select($$;$@)
{
    ...
    while(1)
    {
        ...
        for(my $i = 0; $i < @columns; $i++)
        {
            ...
            if($filter ne "*")
            {
                if($col_name eq $filt[0] && $cont ne $filt[1])
                {
                    $print_row = 0;
                }
            }
        ...
        if($print_row == 1 && $is_deleted == 0)
        {
            ...
            $callback->({row => $row_arr, row_start_position => $start_position, row_end_position => $readed_bytes}, $table_name, $filter, @callback_data);
...
sub UpdateRow($$$@)
{
    my($row, $table_name, $filter, @update_data) = @_;
    ...
    InsertIntoTable($table_name, @update);
...
try {
    ...
    #CreateTable("test4", "id int, id2 int, id3 int, id4 int, id5 int, name text, name2 text, name3 text");     
    
    #Delete("test4", "id=59999");    

    my $rows = Select("test4", "id=50002", \&PrintRow);
    ...
    #Update("test4", "id=50001", "name3=Tzezo2");
    ...
    # {    
    #     InsertIntoTable("test4", "id=$i", "id2=2", "id3=3", "id4=4", "id5=5", "name=$one", "name2=$ten", "name3=$onehundred");
    ...
    # my $z = 0;
    # for(my $i = 1; $i < 60000; $i++)
    # {    
    #     Update("test4", "id=$i", "id2=2", "id3=3", "id4=4", "id5=5", "name=$ten", "name2=$one", "name3=$onehundred");
